...
/**
 * Java portion of final project for CS-HU 310
 * Spring 2019 5W1
 * 
 * @author juanbecerra
 * @author carsonsmith
...
class project {
	...
	/**
	 * Attempts to create a connection with a MySQL database
	...
	private static void CreateItem(String itemCode, String itemDescription, String price) {		
		...
		try {
			pstmt = conn.prepareStatement("Call CreateItem('" + itemCode + "', '" + itemDescription + "'," + price + ");");
	...
	private static void CreatePurchase(String itemCode, int purchaseQuantity) {
		...
		try {
			pstmt = conn.prepareStatement("Call CreatePurchase('" + itemCode + "', " + purchaseQuantity + ");");
	...
	private static void CreateShipment(String itemCode, int shipmentQuantity, Date shipmentDate) {
		...
		try {
			pstmt = conn.prepareStatement("Call CreateShipment('" + itemCode + "', " + shipmentQuantity + ", '" + shipmentDate + "');");
	...
	public static void main(String[] args) {
		try {
			...
			makeConnection("5848", "FinalProject", "cs310pw");
